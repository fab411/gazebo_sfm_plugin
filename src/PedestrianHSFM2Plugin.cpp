...
#include <gazebo_sfm_plugin/PedestrianHSFM2Plugin.h>

using namespace gazebo;
GZ_REGISTER_MODEL_PLUGIN(PedestrianHSFM2Plugin)

#define WALKING_ANIMATION "walking"

/////////////////////////////////////////////////
PedestrianHSFM2Plugin::PedestrianHSFM2Plugin() {}

/////////////////////////////////////////////////
void PedestrianHSFM2Plugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  this->sdf = _sdf;
  this->actor = boost::dynamic_pointer_cast<physics::Actor>(_model);
  this->world = this->actor->GetWorld();

  this->hsfm2Actor.id = this->actor->GetId();

  // Initialize hsfm2Actor position
  ignition::math::Vector3d pos = this->actor->WorldPose().Pos();
  ignition::math::Vector3d rpy = this->actor->WorldPose().Rot().Euler();
  this->hsfm2Actor.position.set(pos.X(), pos.Y());
  this->hsfm2Actor.yaw = utils::Angle::fromRadian(rpy.Z()); // yaw
  ignition::math::Vector3d linvel = this->actor->WorldLinearVel();
  this->hsfm2Actor.velocity.set(linvel.X(), linvel.Y());
  this->hsfm2Actor.linearVelocity = linvel.Length();
  ignition::math::Vector3d angvel = this->actor->WorldAngularVel();
  this->hsfm2Actor.angularVelocity = angvel.Z(); // Length()

  // Read in the maximum velocity of the pedestrian
  if (_sdf->HasElement("velocity"))
    this->hsfm2Actor.desiredVelocity = _sdf->Get<double>("velocity");
  else
    this->hsfm2Actor.desiredVelocity = 0.8;
  ...
  if (_sdf->HasElement("radius"))
    this->hsfm2Actor.radius = _sdf->Get<double>("radius");
  // Read in the target actor mass
  if (_sdf->HasElement("mass"))
    this->hsfm2Actor.mass = _sdf->Get<double>("mass");
  ...
  if (_sdf->HasElement("relaxation_time"))
    this->hsfm2Actor.params.relaxationTime = _sdf->Get<double>("relaxation_time");
  // Read in the k orthogonal
  if (_sdf->HasElement("k_orthogonal"))
    this->hsfm2Actor.params.kOrthogonal =_sdf->Get<double>("k_orthogonal");
  // Read in the k damping
  if (_sdf->HasElement("k_damping"))
    this->hsfm2Actor.params.kDamping = _sdf->Get<double>("k_damping");
  // Read in the k lambda
  if (_sdf->HasElement("k_lambda"))
    this->hsfm2Actor.params.kLambda = _sdf->Get<double>("k_lambda");
  // Read in the alpha
  if (_sdf->HasElement("alpha"))
    this->hsfm2Actor.params.alpha = _sdf->Get<double>("alpha");
  // Read in the group distance forward
  if (_sdf->HasElement("group_distance_forward"))
    this->hsfm2Actor.params.groupDistanceForward = _sdf->Get<double>("group_distance_forward");
  // Read in the group distance othogonal
  if (_sdf->HasElement("group_distance_orthogonal"))
    this->hsfm2Actor.params.groupDistanceOrthogonal = _sdf->Get<double>("group_distance_orthogonal");
  // Read in the k1g
  if (_sdf->HasElement("k1g"))
    this->hsfm2Actor.params.k1g = _sdf->Get<double>("k1g");
  // Read in the k2g
  if (_sdf->HasElement("k2g"))
    this->hsfm2Actor.params.k2g = _sdf->Get<double>("k2g");
  // Read in the Ai
  if (_sdf->HasElement("Ai"))
    this->hsfm2Actor.params.Ai = _sdf->Get<double>("Ai");
  // Read in the Aw
  if (_sdf->HasElement("Aw"))
    this->hsfm2Actor.params.Aw = _sdf->Get<double>("Aw");
  // Read in the Bi
  if (_sdf->HasElement("Bi"))
    this->hsfm2Actor.params.Bi = _sdf->Get<double>("Bi");
  // Read in the Bw
  if (_sdf->HasElement("Bw"))
    this->hsfm2Actor.params.Bw = _sdf->Get<double>("Bw");
  // Read in the k1
  if (_sdf->HasElement("k1"))
    this->hsfm2Actor.params.k1 = _sdf->Get<double>("k1");
  // Read in the k2
  if (_sdf->HasElement("k2"))
    this->hsfm2Actor.params.k2 = _sdf->Get<double>("k2");
  ...
  if (_sdf->HasElement("group")) {
    this->hsfm2Actor.groupId = this->hsfm2Actor.id;
  ...
  this->connections.push_back(event::Events::ConnectWorldUpdateBegin(
      std::bind(&PedestrianHSFM2Plugin::OnUpdate, this, std::placeholders::_1)));

  this->Reset();
}

/////////////////////////////////////////////////
void PedestrianHSFM2Plugin::Reset() {
  this->lastUpdate = 0;
  ...
  if (this->sdf->HasElement("trajectory")) {
    ...
    if (modelElemCyclic)
      this->hsfm2Actor.cyclicGoals = modelElemCyclic->Get<bool>();
    ...
    while (modelElem) {
      ...
      goal.radius = 0.3;
      this->hsfm2Actor.goals.push_back(goal);
...
/////////////////////////////////////////////////
void PedestrianHSFM2Plugin::HandleObstacles() {
  double minDist;
  ignition::math::Vector2d closest_obs;
  ignition::math::Vector2d closest_obs2;
  this->hsfm2Actor.obstacles1.clear();
...
/////////////////////////////////////////////////
void PedestrianHSFM2Plugin::HandlePedestrians() {
  ...
  for (unsigned int i = 0; i < this->world->ModelCount(); ++i) {
    ...
    if (model->GetId() != this->actor->GetId() &&
      ...
      if (pos.Length() < this->peopleDistance) {
        ...
        ped.radius = this->hsfm2Actor.radius;
...
/////////////////////////////////////////////////
void PedestrianHSFM2Plugin::OnUpdate(const common::UpdateInfo &_info) {
  // Time delta
  double dt = (_info.simTime - this->lastUpdate).Double();
  
  if (dt >= this->samplingTime){
    // Get actor actual pose
    ...
    hsfm2::HSFM2.computeForces(this->hsfm2Actor, this->otherActors);

    // Update model
    if (!this->rungeKutta45) {
      hsfm2::HSFM2.updatePosition(this->hsfm2Actor, dt);
    } else {
      hsfm2::HSFM2.updatePositionRKF45(this->hsfm2Actor, _info.simTime.Double(), dt);
    }
    ...
    utils::Angle h = this->hsfm2Actor.yaw;
    ...
    // }
    actorPose.Pos().X(this->hsfm2Actor.position.getX());
    actorPose.Pos().Y(this->hsfm2Actor.position.getY());
...
/////////////////////////////////////////////////
void PedestrianHSFM2Plugin::PublishForces() {
  // std::cout<<"Start of Publish Forces method"<<std::endl;
  gazebo_sfm_plugin::msg::Forces msg = gazebo_sfm_plugin::msg::Forces();
  // Global force
  msg.global_force.x = this->hsfm2Actor.forces.globalForce.getX();
  msg.global_force.y = this->hsfm2Actor.forces.globalForce.getY();
  // Desired force
  msg.desired_force.x = this->hsfm2Actor.forces.desiredForce.getX();
  msg.desired_force.y = this->hsfm2Actor.forces.desiredForce.getY();
  // Obstacle force
  msg.obstacle_force.x = this->hsfm2Actor.forces.obstacleForce.getX();
  msg.obstacle_force.y = this->hsfm2Actor.forces.obstacleForce.getY();
  // Social force
  msg.social_force.x = this->hsfm2Actor.forces.socialForce.getX();
  msg.social_force.y = this->hsfm2Actor.forces.socialForce.getY();
  // Group force
  msg.group_force.x = this->hsfm2Actor.forces.groupForce.getX();
  msg.group_force.y = this->hsfm2Actor.forces.groupForce.getY();
  // Torque force
  msg.torque_force = this->hsfm2Actor.forces.torqueForce;
  // Linear velocity
  msg.linear_velocity.x = this->hsfm2Actor.velocity.getX();
  msg.linear_velocity.y = this->hsfm2Actor.velocity.getY();
  // Angular velocity
  msg.angular_velocity = this->hsfm2Actor.angularVelocity;
  // Pose
  msg.pose.x = this->hsfm2Actor.initPosition.getX();
  msg.pose.y = this->hsfm2Actor.initPosition.getY();
  msg.pose.theta = this->hsfm2Actor.initYaw.toDegree();
