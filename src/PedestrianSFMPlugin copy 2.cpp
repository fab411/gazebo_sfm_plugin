...
void PedestrianSFMPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  //    
///////////////////////
  // Node und Publisher für die Position und Geschwindigkeit
  // Annahme, dass 'node' ein gültiges rclcpp::Node-Objekt ist
  auto node = std::make_shared<rclcpp::Node>("your_node_name");
  
  // Node und Publisher für die Position und Geschwindigkeit
  positionPublisher = node->create_publisher<geometry_msgs::msg::Pose>("position_Actor", 10);
  velocityPublisher = node->create_publisher<geometry_msgs::msg::Twist>("velocity_Actor", 10);

  // Weitere Initialisierungen

  // Annahme, dass sfmActor.position und sfmActor.velocity korrekt aktualisiert werden
  positionMsg.position.x = sfmActor.position.getX();
  positionMsg.position.y = sfmActor.position.getY();
  //positionMsg.position.z = actorPose.Pos().Z(); // Hier wird die Z-Komponente der Pose verwendet

  velocityMsg.linear.x = this->sfmActor.velocity.getX();
  velocityMsg.linear.y = this->sfmActor.velocity.getY();
  //velocityMsg.linear.z = actorPose.Rot().Euler().Z(); // Hier wird die Z-Rotation der Pose verwendet
