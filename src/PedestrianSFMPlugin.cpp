...
void PedestrianSFMPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  ignition::math::Vector3d pos = this->actor->WorldPose().Pos();   // Pose Actor
  ...
  ignition::math::Vector3d linvel = this->actor->WorldLinearVel(); // Velocity Actor
  ...
  // Read in the publish obstacle boolean variable
  if (_sdf->HasElement("publish_obstacle"))
    this->publishObstacle = _sdf->Get<bool>("publish_obstacle");
  else
    this->publishObstacle = false;
  ...
  if (_sdf->HasElement("topic_name")) {
    this->obstacle_topicName = _sdf->Get<std::string>("topic_name");
  }

    // Read in the node name obstacle
  //if (_sdf->HasElement("node_name")) {
   // this->nodeName = _sdf->Get<std::string>("obstacle_node_name");
  //}

   // Read in the actor name obstacle_
  //if (_sdf->HasElement("obstacle_topic_name")) {
  //  this->obstacle_topicName = _sdf->Get<std::string>("obstacle_topic_name");
  //}
  ...
  // Person as a obstacle
  if (this->publishObstacle == true) {
    this->obstacleNode = std::make_shared<rclcpp::Node>(this->obstacle_nodeName);
    this->obstaclePub = this->obstacleNode->create_publisher<gazebo_sfm_plugin::msg::Obstacle>(this->obstacle_topicName, 10);
  }
  // me
...
void PedestrianSFMPlugin::OnUpdate(const common::UpdateInfo &_info) {
  ...
  if (dt >= this->samplingTime){
    ...
    //printf("Velocity: x=%.2f, y=%.2f, z=%.2f\n", velocityMsg.linear.x, velocityMsg.linear.y, velocityMsg.linear.z);
    //printf("Velocity: x=%.2f, y=%.2f\n", velocityMsg.linear.x, velocityMsg.linear.y);
...
void PedestrianSFMPlugin::PublishForces() {
  ...
  // Print the values
  //std::cout << "Pose X: " << msg.pose.x << ", Pose Y: " << msg.pose.y << std::endl;
  //std::cout << "Linear Velocity X: " << msg.linear_velocity.x << ", Linear Velocity Y: " << msg.linear_velocity.y << std::endl;
}


/////////////////////////////////////
// publish Position & Velocity
void PedestrianSFMPlugin::PublishObstacle() {
  std::cout<<"Start of Publish Perosn position & velocity"<<std::endl;
  gazebo_sfm_plugin::msg::Obstacle ObstacleArray = gazebo_sfm_plugin::msg::Obstacle();
  
  
  // Fill the obstacle message
  ObstacleArray.uuid = this->sfmActor.id.GetId(); //GenerateUniqueUUID(); // #uint8[16] uuid
  ObstacleArray.score = 0.05; // ComputeDetectionConfidence(); // Assuming you have a function to compute confidence
  // Pose
  ObstacleArray.position.x = this->sfmActor.initPosition.getX();
  ObstacleArray.position.y = this->sfmActor.initPosition.getY();
  ObstacleArray.position.z = 0.0; // set the z-coordinate as needed
  // id.pose.theta = this->sfmActor.initYaw.toDegree();
  // Linear velocity
  ObstacleArray.velocity.x = this->sfmActor.velocity.getX();
  ObstacleArray.velocity.y = this->sfmActor.velocity.getY();
  ObstacleArray.velocity.z = 0.0; // set the z-velocity as needed
  // Angular velocity - not implemented
  ObstacleArray.size.x = 1.0; // ComputeObstacleSizeX(); // Assuming you have a function to compute size in X
  ObstacleArray.size.y = 1.0;
  ObstacleArray.size.z = 1.0; 

  // Publish the obstacle message
  this->obstaclePub->publish(ObstacleArray);
  //this->forcesPub->publish(id);
  // std::cout<<"End of Publish Forces method"<<std::endl;
}
