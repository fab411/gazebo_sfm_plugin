...
struct Parameters {
  Parameters()
      : relaxationTime(0.5), kOrthogonal(1), kDamping(500), kLambda(0.3), alpha(3),
        ...
        Ai(2000), Aw(2000), Bi(0.08), Bw(0.08), Ci(1000), Cw(1000), Di(0.08), Dw(0.08), k1(120000), k2(240000) {}
...
inline void SocialForceModel::computeObstacleForce(Agent &agent, utils::Map *map) const {
  ...
  if (agent.obstacles1.size() > 0 || agent.obstacles2.size() > 0) {
    ...
    for (unsigned i = 0; i < agent.obstacles1.size(); i++) {
      ...
      agent.forces.obstacleForce += (agent.params.Aw * std::exp((agent.radius - distance)/ agent.params.Bw) + agent.params.k1 * std::max(double(0), agent.radius - distance)) * minDiff.normalized() - agent.params.k2 * std::max(double(0), agent.radius - distance) * delta_v_iw * t_iw;// + agent.params.Cw * std::exp((agent.radius - distance)/ agent.params.Dw) * t_iw;
    }
...
inline void SocialForceModel::computeSocialForce(unsigned index, std::vector<Agent> &agents) const {
  ...
  for (unsigned i = 0; i < agents.size(); i++) {
    ...
    if ((agent.groupId != agents[i].groupId) || (agent.groupId == -1)) agent.forces.socialForce += agent.params.Ci * std::exp((r_ij - d_ij)/agent.params.Di) * t_ij;
  }
}
...
inline void SocialForceModel::computeSocialForce(Agent &me, std::vector<Agent> &agents) const {
  ...
  for (unsigned i = 0; i < agents.size(); i++) {
    ...
    if ((me.groupId != agents[i].groupId) || (me.groupId == -1)) me.forces.socialForce += me.params.Ci * std::exp((r_ij - d_ij)/me.params.Di) * t_ij;
  }
}
