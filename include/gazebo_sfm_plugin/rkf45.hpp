...
// Methods
void first_diff_eq_sfm(double t, double y[], double yp[]);
void second_diff_eq_sfm(double t, double y2[], double y2p[]);
void first_diff_eq_sfm2(double t, double y[], double yp[]);
void second_diff_eq_sfm2(double t, double y2[], double y2p[]);
void first_diff_eq_hsfm(double t, double y[], double yp[]);
void second_diff_eq_hsfm(double t, double y2[], double y2p[]);
double r8_epsilon();
double r8_abs(double x);
double r8_max(double x, double y);
double r8_min(double x, double y);
double r8_sign(double x);
void r8_fehl(void f(double t,double y[],double yp[]),int neqn,double y[],double t,double h,double yp[],
  double f1[],double f2[],double f3[],double f4[],double f5[],double s[]);
int r8_rkf45(void f(double t,double y[],double yp[]),int neqn,double y[],double yp[],double *t,
  double tout,double *relerr,double abserr,int flag );
tuple<utils::Vector2d, utils::Vector2d> rkf45_sfm(utils::Vector2d globalForce, 
    utils::Vector2d init_velocity,utils::Vector2d init_position,double desired_velocity,
    double time,double dt);
tuple<utils::Vector2d, utils::Vector2d> rkf45_sfm2(utils::Vector2d globalForce,double mass, 
    utils::Vector2d init_velocity,utils::Vector2d init_position,double desired_velocity,
    double time,double dt);
tuple<utils::Vector2d,utils::Vector2d,double,utils::Vector2d,utils::Angle> rkf45_hsfm(utils::Vector2d globalForce, 
    double torqueForce,double mass,double inertia,utils::Vector2d init_body_velocity,double init_ang_velocity,
    utils::Vector2d init_position,utils::Angle init_yaw,double desired_velocity,double time,double dt);

//****************************************************************************************************
// SFM - Differential equation of the VELOCITY 
//****************************************************************************************************
void first_diff_eq_sfm(double t, double y[], double yp[]) {
    yp[0] = global_force.getX();
    yp[1] = global_force.getY();
}
//****************************************************************************************************
// SFM - Differential equation of the POSITION
//****************************************************************************************************
void second_diff_eq_sfm(double t, double y2[], double y2p[]) {
    y2p[0] = velocity.getX();
    y2p[1] = velocity.getY();
...
//****************************************************************************************************
// SFM - Runge-Kutta-Fehlberg-45 Integration
//****************************************************************************************************
tuple<utils::Vector2d, utils::Vector2d> rkf45_sfm(utils::Vector2d globalForce, 
    utils::Vector2d init_velocity, utils::Vector2d init_position, double desired_velocity,
    double time, double dt) {
    ...
    double abserr2 = sqrt ( r8_epsilon ( ) );
    double relerr2 = sqrt ( r8_epsilon ( ) );
    ...
    /// STORE VALUES FOR SECOND INTEGRATION
    velocity = utils::Vector2d(y[0],y[1]);
    // Saturate the value of velocity to the desired velocity
    if (velocity.norm() > desired_velocity) {
      velocity = velocity.normalize() * desired_velocity;
    }    
    ...
    /// FINAL VALUES
    utils::Vector2d vel(y[0],y[1]);
    utils::Vector2d pos(y2[0],y2[1]);
    tuple<utils::Vector2d, utils::Vector2d> results(vel,pos);

    return results;
}
//****************************************************************************************************
// SFM2 - Differential equation of the VELOCITY 
//****************************************************************************************************
void first_diff_eq_sfm2(double t, double y[], double yp[]) {
    yp[0] = global_force.getX() / m;
    yp[1] = global_force.getY() / m;
}
//****************************************************************************************************
// SFM2 - Differential equation of the POSITION
//****************************************************************************************************
void second_diff_eq_sfm2(double t, double y2[], double y2p[]) {
    y2p[0] = velocity.getX();
    y2p[1] = velocity.getY();
...
//****************************************************************************************************
// SFM2 - Runge-Kutta-Fehlberg-45 Integration
//****************************************************************************************************
tuple<utils::Vector2d, utils::Vector2d> rkf45_sfm2(utils::Vector2d globalForce, double mass, 
    utils::Vector2d init_velocity, utils::Vector2d init_position, double desired_velocity,
    double time, double dt) {
...
//****************************************************************************************************
// HSFM & HSFM2 - Differential equation of the BODY VELOCITY and ANGULAR VELOCITY
//****************************************************************************************************
void first_diff_eq_hsfm(double t, double y[], double yp[]) {
    yp[0] = global_force.getX() / m; // BV_X
    yp[1] = global_force.getY() / m; // BV_Y
    yp[2] = torque_force / i; // AV
}
//****************************************************************************************************
// HSFM & HSFM2 - Differential equation of the POSITION and YAW
//****************************************************************************************************
void second_diff_eq_hsfm(double t, double y2[], double y2p[]) {
    y2p[0] = body_velocity.getX() * rotationMatrix[0][0] + body_velocity.getY() * rotationMatrix[0][1]; // R_X
    y2p[1] = body_velocity.getX() * rotationMatrix[1][0] + body_velocity.getY() * rotationMatrix[1][1]; // R_Y
    y2p[2] = angular_velocity; // angle
}
//****************************************************************************************************
// HSFM & HSFM2 - Runge-Kutta-Fehlberg-45 Integration
//****************************************************************************************************
tuple<utils::Vector2d, utils::Vector2d, double, utils::Vector2d, utils::Angle> rkf45_hsfm(utils::Vector2d globalForce, 
    double torqueForce, double mass, double inertia, utils::Vector2d init_body_velocity, double init_ang_velocity,
    utils::Vector2d init_position, utils::Angle init_yaw, double desired_velocity, double time, double dt) {
    ...
    /// INITIALIZE PARAMETERS
    rotationMatrix[0][0] = init_yaw.cos();
    rotationMatrix[0][1] = -init_yaw.sin();
    rotationMatrix[1][0] = init_yaw.sin();
    rotationMatrix[1][1] = init_yaw.cos();
    double t = time;
    double t2 = time;
    double t_out = t + dt;
    double t2_out = t2 + dt;
    ...
    /// FIRST INTEGRATION
    // Initial values
    double y[3] = {init_body_velocity.getX(), init_body_velocity.getY(), init_ang_velocity};
    first_diff_eq_hsfm(t, y, yp);
    // Integrate
    flag1 = r8_rkf45(first_diff_eq_hsfm, neqn, y, yp, &t, t_out, &relerr, abserr, flag1);

    /// STORE VALUES FOR SECOND INTEGRATION
    body_velocity = utils::Vector2d(y[0],y[1]);
    angular_velocity = y[2];
    // Saturate the value of body velocity to the desired velocity
    if (body_velocity.norm() > desired_velocity) {
      body_velocity = body_velocity.normalize() * desired_velocity;
    }    

    /// SECOND INTEGRATION
    // Initial values
    double y2[3] = {init_position.getX(), init_position.getY(), init_yaw.toRadian()};
    second_diff_eq_hsfm(t2, y2, yp2);
    // Integrate
    flag2 = r8_rkf45(second_diff_eq_hsfm, neqn, y2, yp2, &t2, t2_out, &relerr2, abserr2, flag2);

    /// FINAL VALUES
    utils::Vector2d bVel(y[0],y[1]);
    utils::Vector2d vel(bVel * rotationMatrix);
    double ang_vel = y[2];
    utils::Vector2d pos(y2[0],y2[1]);
    utils::Angle yaw(y2[2]);
    tuple<utils::Vector2d, utils::Vector2d, double, utils::Vector2d, utils::Angle> results(bVel,vel,ang_vel,pos,yaw);
...
//****************************************************************************************************
// Returns the max between two R8
//****************************************************************************************************
double r8_max ( double x, double y )
{
  if ( y < x )
...
//****************************************************************************************************
// Returns the min between two R8
//****************************************************************************************************
double r8_min ( double x, double y )
{
  if ( y < x )
...
//****************************************************************************************************
// Takes one Runge-Kutta-Fehlberg fourth-fifth order step.
//****************************************************************************************************
